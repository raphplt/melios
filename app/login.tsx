	...
	const signInWithGoogle = async () => {
		try {
			...
			const querySnapshot = await getDocs(
				query(membersCollectionRef, where("uid", "==", user.uid))
			);

			// Create member document only if it doesn't exist
	...
	const signInWithApple = async () => {
		try {
			...
			if (!appleCredential.identityToken) {
				throw new Error(
					"Apple Sign-In a échoué : aucun token d'identité retourné."
				);
			}
			...
			try {
				...
				if (error instanceof Error)
					throw new Error(
						"Erreur lors de l'ajout de l'utilisateur à Firestore: " + error.message
					);
			}
			...
			try {
				...
				if (error instanceof Error)
					throw new Error(
						"Erreur lors de la sauvegarde des données utilisateur en AsyncStorage: " +
							error.message
					);
			}
